...
export default function GenAITracker({ store }) {
  const { genaiTopics, updateGenAITopic, genaiStats } = store;
  const [expandedCats, setExpandedCats] = useState(new Set(['llm-foundations']));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">GenAI & Agentic AI</h1>
        <p className="text-text-secondary text-sm mt-1">
          {genaiStats.completed}/{genaiStats.total} topics completed |
          LLM Foundations → APIs → RAG → Fine-tuning → Agents
        </p>
      </div>
      ...
      {genaiTopics.map(cat => {
        ...
        const catStats = genaiStats.byCategory[cat.name] || { total: 0, completed: 0 };
                        ...
                        <div className="px-3 pb-3">
                          {sec.topics.map(topic => (
                            <div key={topic.id} className="flex items-center gap-3 py-2 border-t border-border/30 first:border-t-0">
                              <button
                                onClick={() => updateGenAITopic(cat.id, sec.id, topic.id, { completed: !topic.completed })}
                              ...
                              <div className="flex gap-1">
                                {[1, 2, 3, 4].map(level => (
                                  <button
                                    key={level}
                                    onClick={() => updateGenAITopic(cat.id, sec.id, topic.id, { mastery: topic.mastery === level ? 0 : level })}
